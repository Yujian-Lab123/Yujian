      ...
      <section className="relative z-10 mx-auto max-w-6xl px-4 pb-12 sm:px-6 sm:pb-16" aria-labelledby="success-title" aria-live="polite">
        <div className={`card-blue px-4 py-8 sm:px-6 sm:py-10 md:px-12 md:py-12 ${styles.successCard}`}>
          ...
          <div className={`fade-up-3 mt-10 flex flex-col justify-center gap-3 md:mt-12 md:flex-row ${styles.actions}`}>
            <a href={`https://zhihu.com/mock/${other.id}`} target="_blank" rel="noreferrer" className="btn-outline-blue">👤 去 TA 的知乎主页</a>
            <button className="btn-outline-blue">＋ 关注 TA</button>
            <a href={`https://zhihu.com/mock/${other.id}`} target="_blank" rel="noreferrer" className="btn-primary-blue">💬 开始交流</a>
          </div>
          <p className="mt-6 text-center text-xs tracking-widest2 text-gold-500">🔒 尊重彼此 · 真诚交流 · 安心连接</p>
        </div>

        <footer className="mt-8 flex flex-col items-center justify-between gap-3 text-center text-xs text-sumi-400 md:flex-row md:text-left">
          <p className="flex flex-wrap items-center justify-center gap-2 md:justify-start"><Logo tone="blue" /> <span>知乎出品 · 算法匹配由「知心引擎」提供支持</span></p>
  ...
  return (
    <div className={styles.successMark} aria-hidden="true">
      ...
      <svg viewBox="0 0 120 44" fill="none">
        ...
        <circle className={styles.connectionDotOne} cx="34" cy="18" r="2.5" />
        <circle className={styles.connectionDotTwo} cx="88" cy="25" r="2.5" />
      </svg>
...
function PersonCard({ p }: { p: any }) {
  return (
    <div className="fade-up-2 flex flex-col items-center text-center">
      <InkAvatar name={p.name} tone="blue" size={112} />
      ...
      <div className={`${styles.personTags} mt-4 flex flex-wrap justify-center gap-2`}>
        {p.tags.map((t: string) => <span key={t} className="chip-blue">{t}</span>)}
      </div>
      <p className={`${styles.personMeta} mt-4 text-xs text-sumi-400`}>知乎 {p.zhihu_years} 年 · 获赞 {p.upvotes}</p>
    </div>
  );
}
