    ...
    <main className={`relative min-h-[100dvh] overflow-hidden bg-paper-100 paper-texture ${styles.page}`}>
      ...
      <section className="relative z-10 mx-auto max-w-4xl px-4 pb-24 sm:px-6">
        <div className={styles.personReveal}>
          ...
          <p className="mt-3 text-center text-sm leading-6 text-sumi-500 sm:text-base">{rec.target.role} · {rec.target.city} · 知乎 {rec.target.zhihu_years} 年 · 获赞 {rec.target.upvotes}</p>
          ...
          <p className="mt-5 text-center font-display text-lg leading-7 text-sumi-700">「 {rec.target.quote} 」</p>
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            {rec.target.tags.map((t: string) => <span key={t} className="chip-warm">{t}</span>)}
          </div>
        </div>

        <div className="card-warm fade-up-2 mt-10 p-5 sm:p-7">
