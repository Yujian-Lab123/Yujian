...
原始内容(RawContent[])
  ...
  → 长文按片段输入（默认每片 3000 字，单篇最多 12000 字）
  → [阶段一·解析模型 qwen3.7-flash] 逐篇摘要 + Topic / Key Question + 候选线索(0~4 条/篇,六类 kind)
  → [阶段二·汇总模型 qwen3.8-flash] 压缩漏斗(删弱证据→并同义→提上位模式→按六标准排序)
  → zod 校验(失败带错误重试一次)
  → 证据消毒(evidence_ids 必须真实存在;无证据结论删除;unknown 结论挪入 unknowns;按 LIMITS 裁剪)
  → [Embedding Provider] Profile 三路向量 + 内容向量（未配置时安全降级）
...
| `lib/profile/engine.ts` | 管线主体:`analyzeProfile(raw, opts)`；支持上一版产物驱动的增量更新 |
| `lib/profile/incremental.ts` | 内容哈希 / 差异检测 / 长文分块 / 向量输入 |
| `lib/providers/embedding.ts` | OpenAI 兼容 Embedding Adapter；批量、校验、脱敏日志 |
...
- **抽取缓存与增量更新**:默认写 `data/crawler/.extract-cache.json`,键 = 提示词版本+模型+分块参数+归一化内容哈希。CLI / Worker 也会读取上一版画像产物；内容不变时复用摘要/线索/向量并跳过画像综合，只有新增或修改内容进入便宜模型。换模型、改提示词或改分块参数会自动失效。`--cache <path>` 可换位置,`--max-items N` 限制单人分析条数(默认 80,超出取最新 N 篇)。
...
- 单条正文默认最多使用 12000 字(`--max-chars` / `opts.maxTextChars`)，按 3000 字分块(`--chunk-chars` / `opts.chunkChars`)；候选线索上限 400 条。
- Embedding 使用 `EMBED_BASE_URL / EMBED_API_KEY / EMBED_MODEL / EMBED_DIMENSIONS`；P5 固定采用 1024 维，画像产物保存成功后由 Worker 幂等同步到 pgvector。未配置、维度不兼容或同步失败时画像仍可生成，在线匹配回退 16 维 Mock，状态见 `meta.embedding` 和任务消息。
