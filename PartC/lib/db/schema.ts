...
export const RETRIEVAL_EMBEDDING_KINDS = [
  'profile_long_term',
  'profile_value',
...
export type RetrievalEmbeddingKind = (typeof RETRIEVAL_EMBEDDING_KINDS)[number];
...
/**
 * P5 统一向量索引：Profile 四路与 Content Anchor 共用同一固定维度向量列。
 * HNSW 索引由自定义迁移创建；这里保留 Drizzle 的列与关系契约。
 */
export const retrievalEmbeddings = pgTable('retrieval_embeddings', {
  id: text('id').primaryKey(),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  kind: text('kind').$type<RetrievalEmbeddingKind>().notNull(),
  ...
  uniqueIndex('retrieval_embeddings_owner_kind_uidx').on(table.userId, table.kind, table.sourceId),
  index('retrieval_embeddings_space_kind_idx').on(table.spaceId, table.kind, table.userId),
]);
