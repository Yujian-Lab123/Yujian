...
export function renderReport(a: ProfileArtifact): string {
  ...
  if (m.incremental) {
    L.push(`增量更新：新增 ${m.incremental.added} / 修改 ${m.incremental.changed} / 未变 ${m.incremental.unchanged} / 删除 ${m.incremental.removed} · 画像${m.incremental.profile_reused ? '复用' : '已更新'}`);
  }
  if (m.embedding) {
    L.push(`Embedding：${m.embedding.status}${m.embedding.model ? ` · ${m.embedding.model}` : ''}${m.embedding.dimensions ? ` · ${m.embedding.dimensions} 维` : ''}`);
  }
  L.push('');
