...
// ---- P4：逐内容摘要（与候选线索同一次便宜模型调用产生） ----

export const ContentAnalysisSchema = z.object({
  content_id: z.string(),
  summary: z.string().trim().min(1).max(1_000),
  topics: z.array(z.string().trim().min(1).max(60)).max(8),
  key_questions: z.array(z.string().trim().min(1).max(200)).max(5),
  candidates: z.array(CandidateSchema).max(4),
});
export type ContentAnalysis = z.infer<typeof ContentAnalysisSchema>;

export const ContentAnalysisBatchSchema = z.object({ analyses: z.array(ContentAnalysisSchema) });

export interface ContentAnalysisRecord extends ContentAnalysis {
  /** 归一化内容的稳定哈希，用来判断新增、修改和可直接复用的内容。 */
  source_hash: string;
}
...
export interface ProfileArtifact {
  ...
  meta: {
    ...
    incremental?: {
      ...
      unchanged: number;
      removed: number;
      reused_analyses: number;
      reused_embeddings: number;
      profile_reused: boolean;
    };
    embedding?: {
      status: 'generated' | 'reused' | 'not_configured' | 'failed';
  ...
  /** P4 逐内容摘要/主题/问题及其源内容哈希；供增量更新和可追溯向量输入复用。 */
  content_analyses?: ContentAnalysisRecord[];
  /** P4 真实向量产物。P5 才会将其接入 pgvector / ANN 在线检索。 */
  embeddings?: {
    ...
    contents: Array<{ content_id: string; source_hash: string; vector: number[] }>;
  };
}
