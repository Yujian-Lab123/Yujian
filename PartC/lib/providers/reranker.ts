// ============ Rerank Provider（Qwen compatible / DashScope legacy） ============
// P6 只向排序服务发送压缩后的结构化画像，不发送原始内容、证据正文或密钥。
...
export function buildRerankRequest(input: {
  ...
  const topN = input.documents.length;
...
export function parseRerankResponse(payload: unknown, documentCount: number): { scores: number[]; ranking: number[]; totalTokens?: number } {
  ...
  for (const item of results) {
    ...
    if (typeof index !== 'number' || !Number.isInteger(index) || index < 0 || index >= documentCount) {
      throw new Error(`Rerank 响应包含越界 index：${String(item.index)}`);
    }
    if (scores[index] !== null) throw new Error(`Rerank 响应包含重复 index：${index}`);
    if (typeof score !== 'number' || !Number.isFinite(score) || score < 0 || score > 1) {
      throw new Error(`Rerank 响应包含无效分数：${String(item.relevance_score)}`);
    }
    scores[index] = score;
    ranking.push(index);
  }
  ...
  const rawTotalTokens = body.usage?.total_tokens;
  return {
...
/** 调用真实排序模型；配置缺失、网络失败或响应不完整时返回 null，由匹配管线整批回退。 */
export async function rerankTexts(
  ...
  try {
    const response = await (options.fetchImpl || fetch)(rerankEndpoint(baseUrl, apiStyle), {
      ...
      body: JSON.stringify(buildRerankRequest({
        model,
        apiStyle,
        query,
        documents,
