...
function ranked(id: string, mutual: number): RankedCandidate<RerankUser> {
  return {
    user: { ...user, id },
...
describe('model rerank integration helpers', () => {
  ...
  it('replaces mock rerank scores and recomputes final ordering', () => {
  ...
  it('reserves a paid-pool slot for a strong Current State match', () => {
    const ordinary = [ranked('a', 0.9), ranked('b', 0.8), ranked('c', 0.7)];
    ordinary[2].cur = 0.8;
    expect(selectRerankPool(ordinary, 2).map((item) => item.user.id)).toEqual(['a', 'c']);
  });
});
