...
import { looseParseItems, readArtifactFile, saveArtifactFiles, slugifyName } from '../lib/profile/store.ts';
...
async function main() {
  ...
  if (flags.fixture) {
    ...
    console.error('用法:node --env-file=.env.local scripts/analyze-profile.ts --input <file.json> [--name "某人"] [--author "作者名"] [--peek] [--out dir] [--max-chars N] [--chunk-chars N]\n      node --env-file=.env.local scripts/analyze-profile.ts --fixture');
    process.exit(1);
  }
  ...
  const previousArtifact = readArtifactFile(outDir, slugifyName(name || 'person'));
  const artifact = await analyzeProfile(raws, {
    ...
    chunkChars: typeof flags['chunk-chars'] === 'string' ? Number(flags['chunk-chars']) : undefined,
  ...
  增量:${artifact.meta.incremental ? `新增 ${artifact.meta.incremental.added} / 修改 ${artifact.meta.incremental.changed} / 未变 ${artifact.meta.incremental.unchanged} / 删除 ${artifact.meta.incremental.removed}；画像${artifact.meta.incremental.profile_reused ? '复用' : '已更新'}` : '无'}
  Embedding:${artifact.meta.embedding ? `${artifact.meta.embedding.status}${artifact.meta.embedding.dimensions ? ` / ${artifact.meta.embedding.dimensions} 维` : ''}` : '未执行'}
