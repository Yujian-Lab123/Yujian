...
`/demo/profile` 在本地研究环境可读取白名单中的 `data/crawler/<id>.json` 与对应的
`profile-output/<id>.profile.json`，复用正式画像地图展示代表内容和证据；`/demo/content`
只兼容重定向，不再是独立内容目录。原始采集输入与画像产物仍不进入 Git 或镜像。

生产环境即使挂载了这两个目录，也默认不公开真人画像。只有确认答主明确同意
**整份 AI 推断画像**用于公开演示后，才逐人设置服务端 `DEMO_PUBLIC_PROFILE_IDS`
（逗号分隔白名单 id）；公开内容本身的许可不能替代此同意。未设置或缺少任一文件时
仅展示演示身份自己的画像（如果已生成）或明确空状态，不回退到任意本地产物。

真实 `/profile` 始终只按真实会话读取本人画像；真实相遇的召回/排序/落库也只使用
非 Mock 候选，Demo 相遇只使用 Mock 候选。API 返回前的模式过滤仍作为第二道保护。
