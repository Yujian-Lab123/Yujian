...
当前按三名实际成员协作：A 负责此刻状态，C 负责相遇与匹配，D 负责侧面、长期画像、共享平台与集成。`/side` 是 D 的正式目录，不存在 B 角色或 B 审批人。
...
| D：产品总控 / Side / 基座 / 集成 | `/side`、Landing、引导、长期画像、共享平台 | `app/side/**`、`lib/contextual-persona.ts`、`lib/contextual-persona.test.ts`、`app/page.tsx`、`app/onboarding/**`、`app/profile/**`、`app/api/profile/**`、`lib/profile/**`、`lib/db/**`、`lib/providers/**`、`lib/session.ts`、`app/api/auth/**`、`drizzle/**`、`public/**`、发布文档 | A/C 的业务页面细节 |

## 明确禁止的重叠

- A 不修改 `/side` 的解释规则；D 不修改 `/me` 的记录表单和状态写入接口。
