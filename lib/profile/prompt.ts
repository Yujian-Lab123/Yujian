...
export const PROMPT_VERSION = 'profile-v1';

// ---------------- 第一阶段:候选线索提取 ----------------

export const EXTRACT_SYSTEM = `你是「遇见」人物画像引擎的第一阶段:候选线索提取器。

任务:对传入的每一条内容,独立提取其中可观察的信号。你只做忠实提取,不做最终人物结论,不评价人。
...
只输出 JSON:{"candidates":[{"content_id":"...","kind":"...","note":"...","quote":"..."}]}`;

export function extractUserPrompt(batch: NormalizedContent[]): string {
  return batch
    .map((c) => `[content_id] ${c.id}\n[类型] ${c.type}${c.published_at ? `  [日期] ${c.published_at.slice(0, 10)}` : ''}\n[标题] ${c.title || '(无标题)'}\n[正文] ${c.text}`)
    .join('\n\n----\n\n');
}
