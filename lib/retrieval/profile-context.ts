...
/** 同一用户可能有多个历史画像，按更新时间取最新一份。 */
export async function loadLatestProfileArtifacts(userIds: string[]): Promise<Map<string, ProfileArtifact>> {
  const ids = [...new Set(userIds.filter(Boolean))];
  if (ids.length === 0) return new Map();
  const rows = await db.select({ userId: profileArtifacts.userId, artifact: profileArtifacts.artifact })
  ...
  for (const row of rows) {
    if (row.userId && !result.has(row.userId)) result.set(row.userId, row.artifact as unknown as ProfileArtifact);
  }
  return result;
}
